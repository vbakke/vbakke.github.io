...
(function () {
    var root = this
    var previous_trytes = root.trytes
    ...
    const TRYTEBOM_ENCODING = {
        'YZ': ['utf-16le', 'utf16le'],
        'ZY': ['utf-16be', 'utf16be'],
        'YY': ['utf-8', 'utf8'],
        '': ['windows-1252', 'latin1'],
    }
    const ENCODING_TRYTEBOM = {}; // Reverse of TRYTEBOM_ENCODING. Dynamically initialized in getBom()
     ...
     */
    trytes.decodeBytesFromTryteString = function(inputTrytes) {
     ...
     */
    trytes.encodeTryteStringAsBytes = function(tryte3Str) {
    ...
    trytes.encodeTextAsTryteString = function (text, encoding) {
        ...
        let tryte3Str = trytes.encodeBytesAsTryteString(bytes);
     ...
     */
    trytes.decodeTextFromTryteString = function(tryte3Str, encoding) {
        // Check if BOM exists
        let bom = tryte3Str.substr(0, 2);
        if (bom in TRYTEBOM_ENCODING) {
            ...
            if (typeof encoding === 'undefined') {
                encoding = getEncodingFromBom(bom);
            }
        }
        if (typeof encoding === 'undefined') {
        ...
        let bytes = trytes.decodeBytesFromTryteString(tryte3Str);
        let text = trytes.decodeTextFromBytes(bytes, encoding);
        return text;
    }



    trytes.encodeTextAsBytes = function(text, encoding) {
        if (typeof encoding === 'undefined') {
            encoding = selectBestEncoding(text);
        }
        ...
        if (isBrowser) {
            ...
            bytes = Buffer.from(text, encoding);
        }

        return bytes;
    }

    trytes.decodeTextFromBytes = function(bytes, encoding) {
        let text;
        if (isBrowser) {
            if (encoding === 'latin1') encoding = 'windows-1252';
    ...
    function selectBestEncoding(text) {
        if (containsAsciiOnly(text)) {
            return (isBrowser) ? 'windows-1252' : 'latin1';
        }
        else if (isBrowser) {
            return 'utf-8';
        } 
    ...
    function getEncodingFromBom(bom) {
        let i = (isBrowser) ? 0 : 1;
        return TRYTEBOM_ENCODING[bom][i];
    }

    function getBomFromEncoding(encoding) {
        if (! (encoding in ENCODING_TRYTEBOM) ) {
            reverseTryteBom();
        }
        return ENCODING_TRYTEBOM[encoding];
    }

    function reverseTryteBom() {
        for (let bom in TRYTEBOM_ENCODING) {
            for (var i in TRYTEBOM_ENCODING[bom]) {
                ENCODING_TRYTEBOM[TRYTEBOM_ENCODING[bom][i]] = bom;
            }
